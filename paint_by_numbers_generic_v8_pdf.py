...
def build_palette_matrix(names):
    ...
    return np.stack(cols, axis=1)  # (3, N)

# ---------- Mixing solver ----------
def solve_mix_weights(target_rgb, names):
    A = build_palette_matrix(names)      # (3, N) linear RGB
    b = srgb_to_linear_arr(np.array(target_rgb, dtype=float))  # (3,)
    w, _ = nnls(A, b)  # non-negative least squares
    if w.sum() == 0:
        w = np.ones(len(names))
    w = w / w.sum()
    recon_lin = A @ w
...
# ---------- Drawing helpers ----------
def draw_color_key(ax, palette, recipes, entries_per_color, base_palette, used_indices=None, title="Color Key • Ratios + Component Paints"):
    ...
    for row_idx, ci in enumerate(used_indices):
        ...
        ax.add_patch(Rectangle((0, row_idx), 1, 1, color=(color/255)))
        ax.text(1.3, row_idx+0.5, f"{ci+1}: {recipe}", va="center", fontsize=8)
        base_x = 12.0
        for j, (comp_name, _) in enumerate(entries):
            comp_rgb = np.array(base_palette[comp_name]) / 255.0
            ax.add_patch(Rectangle((base_x + j*1.1, row_idx), 1, 1, color=comp_rgb, ec="k", lw=0.2))
    ax.set_xlim(0, 16)
...
# ---------- Main ----------
def main():
    parser = argparse.ArgumentParser(description="Generate A4 landscape multi-page PDF with per-frame color keys (ratios + component paints).")
    ...
    parser.add_argument("--csv", default=None, help="Optional CSV to save computed integer parts per base paint")
    ...
    # Compute recipes & weights
    names = args.palette
    all_weights, all_errors, all_entries, all_recipes = [], [], [], []
    for col in palette:
        w, err = solve_mix_weights(col, names)
        entries = simplify_parts(w, names, max_components=args.components, scale=12)
        rec = recipe_text(entries)
        all_weights.append(w); all_errors.append(err); all_entries.append(entries); all_recipes.append(rec)

    # Auto progress groups
    ...
    # ----- Build PDF (A4 Landscape: 11.69 x 8.27 inches) -----
    A4_LANDSCAPE = (11.69, 8.27)

    from matplotlib.backends.backend_pdf import PdfPages
    with PdfPages(args.pdf) as pdf:
        # Page 1: Original above PBN (left col), overall Key (right col spanning both rows)
        fig = plt.figure(figsize=A4_LANDSCAPE)
        gs = GridSpec(2, 2, width_ratios=[1,1.4], figure=fig)
        ...
        draw_color_key(ax3, palette, all_recipes, all_entries, BASE_PALETTE, used_indices=list(range(args.colors)),
                       title="Color Key • All Clusters")

        plt.tight_layout()
        pdf.savefig(fig, dpi=300); plt.close(fig)

        # Subsequent pages: per-frame image + per-frame key (only clusters used in that frame)
        for title, idxs, frame in progress_frames:
            ...
            draw_color_key(axR, palette, all_recipes, all_entries, BASE_PALETTE, used_indices=idxs,
                           title=f"Color Key • {title}")

            plt.tight_layout()
            pdf.savefig(fig, dpi=300); plt.close(fig)

    print(f"✅ Saved A4 landscape multi-page PDF with per-frame keys to {args.pdf}")
